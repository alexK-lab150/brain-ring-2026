...
function QuestionEditor({ question, onSave }) {
  ...
  return (
    <div className="space-y-3">
      {field("Вопрос (RU)", "textRu", "white", 3)}
      {field("Вопрос (UA)", "textUa", "white", 3)}
      {field("Ответ", "answer", "gold", 1)}
...
function TeamRow({ team, rank, onDelta, onSetScore, onRename, onRemove }) {
  ...
  return (
    <div className="flex items-center gap-2 bg-gray-900 border border-gray-800 rounded-xl px-3 py-2">
      <span className="w-6 text-center text-sm font-bold" style={{ color: rankColors[rank] || "#6b7280" }}>
        {rank + 1}
      </span>
      {editName ? (
        <input ref={nameRef}
        ...
        <button className="flex-1 text-left text-sm text-gray-200 hover:text-white truncate"
      ...
      <button onClick={() => onDelta(team.id, -1)}
        className="w-8 h-8 flex items-center justify-center rounded-lg bg-red-950 hover:bg-red-800 text-red-300 font-bold text-lg transition-colors select-none">−</button>
      <button onClick={() => onDelta(team.id, 1)}
        className="w-8 h-8 flex items-center justify-center rounded-lg bg-green-950 hover:bg-green-700 text-green-300 font-bold text-lg transition-colors select-none">+</button>
      <button onClick={() => onRemove(team.id)}
        className="w-7 h-7 flex items-center justify-center rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-500 hover:text-red-400 text-sm transition-colors" title="Удалить">✕</button>
    </div>
  );
}
...
function BlitzPanel() {
  ...
  return (
    <>
      ...
      {/* Tabs */}
      <nav className="border-b border-gray-800 px-4 flex gap-1 pt-2">
        {tabs.map(({ key, label }) => (
          <button key={key} onClick={() => setTab(key)}
            className={`px-4 py-2 rounded-t-lg text-sm font-medium transition-colors
              ${tab === key ? "bg-gray-800 text-white border-b-2 border-blue-500" : "text-gray-400 hover:text-white"}`}>
            {label}
          </button>
        ))}
      </nav>
      ...
      <main className="flex-1 overflow-y-auto p-4">
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            {!q ? (
              <div className="text-center py-24 text-gray-600">
                ...
                <p>Загрузите файл вопросов через кнопку выше.</p>
              </div>
            ) : (
              <>
                <div className="flex items-center gap-3">
                  <button onClick={() => emit("question:navigate", "prev")} disabled={currentIndex === 0}
                    className="px-5 py-2.5 bg-gray-800 hover:bg-gray-700 disabled:opacity-30 rounded-xl font-semibold transition-colors">
                    ← Назад
                  </button>
                  ...
                  <button onClick={() => emit("question:navigate", "next")} disabled={currentIndex === questions.length - 1}
                    className="px-5 py-2.5 bg-gray-800 hover:bg-gray-700 disabled:opacity-30 rounded-xl font-semibold transition-colors">
                    Вперёд →
                  </button>
                </div>
                ...
                <div className="bg-gray-900 border border-gray-700 rounded-2xl p-4 space-y-3">
                  <p className="text-xs text-gray-500 uppercase tracking-widest">👁 Превью ведущего</p>
                  ...
                  <div className="border-t border-gray-700 pt-3 space-y-1.5">
                    <p className="text-yellow-400 font-bold">✅ {q.answer}</p>
                    <p className="text-gray-400 text-sm">📖 {q.answerSynodal}</p>
                    <p className="text-gray-400 text-sm">📗 {q.answerOgienko}</p>
                  </div>
                </div>

                <button onClick={() => emit("answer:toggle", !showAnswer)}
                  className={`w-full py-3 rounded-xl font-bold text-base transition-all
                    ${showAnswer ? "bg-yellow-600 hover:bg-yellow-500 text-white" : "bg-gray-800 hover:bg-gray-700 text-gray-300"}`}>
                ...
                {q.type === "blitz" && (
                  <div className="bg-gray-900 border border-orange-900 rounded-2xl p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <p className="text-orange-400 font-semibold text-sm uppercase tracking-wide">⚡ Блиц — попытки</p>
                      <button onClick={() => emit("blitz:reset")}
                        className="text-xs px-3 py-1 bg-gray-800 hover:bg-gray-700 rounded-lg text-gray-400 transition-colors">
                        Сбросить
                      </button>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      {[{ key: "attempt1", label: "Попытка 1" }, { key: "attempt2", label: "Попытка 2" }].map(({ key, label }) => (
                        <button key={key}
                    ...
                    {bothFailed && (
                      <div className="bg-red-950 border border-red-700 rounded-xl p-3 text-center">
                        ...
                        <p className="text-red-300 text-sm">На экране — оверлей «ВОПРОС СНЯТ»</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
                  <p className="text-xs text-gray-500 uppercase tracking-widest mb-3">✏️ Редактирование</p>
        ...
        {tab === "teams" && (
          <div className="max-w-xl mx-auto space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold text-gray-300">Команды ({teams.length})</h2>
              <div className="flex gap-2">
                <button onClick={() => { if (window.confirm("Сбросить все очки до 0?")) emit("teams:reset-scores"); }}
                  className="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-red-400 rounded-lg text-xs font-semibold transition-colors">
                  🔄 Сбросить
                </button>
                <button onClick={() => emit("team:add", "")}
            ...
            {sortedTeams.map((team, rank) => (
              <TeamRow key={team.id} team={team} rank={rank}
            ...
            {teams.length === 0 && (
              <div className="text-center py-10 text-gray-600">Нет команд. Нажмите «+ Добавить».</div>
            )}
          </div>
        )}
        ...
        {tab === "list" && (
          <div className="max-w-2xl mx-auto">
            {questions.length === 0 ? (
              <div className="text-center py-24 text-gray-600">Вопросы не загружены.</div>
            ) : (
...
export default function AdminView() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col text-white" style={{ fontFamily: "Inter, sans-serif" }}>

      {/* ── Header ── */}
      <header className="sticky top-0 z-50 bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          <span className="text-2xl">🎯</span>
          <div>
            <h1 className="font-bold text-base leading-none">Brain-Ring Admin</h1>
            <p className="text-xs text-gray-500 mt-0.5">
              {mode === "blitz" ? "Режим: Блиц" : "Режим: Конкурс знатоков"}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex rounded-lg overflow-hidden border border-gray-700">
            <button
              onClick={() => emit("mode:set", "blitz")}
              className={`px-3 py-1.5 text-xs font-semibold transition-colors
                ${mode === "blitz" ? "bg-blue-700 text-white" : "bg-gray-800 text-gray-400 hover:text-white"}`}
            >
              ⚡ Блиц
            </button>
            <button
              onClick={() => emit("mode:set", "captains")}
              className={`px-3 py-1.5 text-xs font-semibold transition-colors
                ${mode === "captains" ? "bg-purple-700 text-white" : "bg-gray-800 text-gray-400 hover:text-white"}`}
            >
              🎓 Знатоки
            </button>
          </div>
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors whitespace-nowrap
              ${soundEnabled ? "bg-green-900 hover:bg-green-800 text-green-300" : "bg-gray-800 hover:bg-gray-700 text-gray-500"}`}
          >
