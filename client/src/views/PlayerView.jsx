  ...
  return (
    ...
    >
      <div className="text-center select-none">
        <p
          className="font-black uppercase tracking-[0.15em] drop-shadow-2xl"
...
export default function PlayerView() {
  const { gameState, soundEnabled } = useSocket(); // ← берём из контекста
  const { playSuccess } = useAudio();
  const prevTsRef = useRef(null);
  const [scorePopId, setScorePopId] = useState(null);

  useEffect(() => {
    console.log("[Player] soundEnabled changed:", soundEnabled);
  }, [soundEnabled]);

  useEffect(() => {
    if (!gameState?.lastScoreChange) return;
    const { teamId, delta, ts } = gameState.lastScoreChange;
    console.log("[Player] score change — delta:", delta, "ts:", ts, "prevTs:", prevTsRef.current, "soundEnabled:", soundEnabled);
    if (delta > 0 && ts !== prevTsRef.current) {
      prevTsRef.current = ts;
      console.log("[Player] calling playSuccess with soundEnabled:", soundEnabled);
    ...
    >
      ...
      <div className="flex items-center gap-3 px-6 py-3 bg-black/40 border-b border-white/5 flex-wrap">
        {sortedTeams.map((team, i) => {
          ...
          const pop = scorePopId === team.id;
          return (
            <div
              key={team.id}
              className={`flex items-center gap-1.5 px-3 py-1 rounded-full ${i < 3 ? "bg-white/10" : "bg-white/5"}`}
            >
              <span className="text-xs font-bold" style={{ color: topColors[i] || "#6b7280" }}>
                #{i + 1}
              </span>
              <span className="text-sm text-gray-300 font-medium">{team.name}</span>
              <span
                className={`font-black text-base ml-1 transition-transform ${pop ? "score-pop" : ""}`}
                style={{ color: topColors[i] || "#9ca3af" }}
              >
                {team.score}
              </span>
            </div>
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              {q.type === "blitz" && (
                <span
                  ...
                  style={{ background: "#7c2d00", color: "#fdba74" }}
                >
                  ⚡ Блиц
                </span>
              )}
